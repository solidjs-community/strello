...
export const fetchBoard = cache(async (boardId: number) => {
    ...
    return {
        ...
        notes: boardFromDataBase?.items.map(note => ({
            ...note,
            board: String(note.boardId),
            column: note.columnId,
            body: note.title || ''
        })) || [],
        columns: boardFromDataBase?.columns.map(column => ({
            ...column,
...
export const createNote: Actions["createNote"] = action(async ({ id, column, body, order, timestamp, board }) => {
    "use server";
    const accountId = await getAuthUser();
    const mutation = {
        id: String(id),
...
export const editNote: Actions["editNote"] = action(async (id, content, timestamp) => {
    "use server";
    const accountId = await getAuthUser();
    const mutation = {
        id: String(id),
...
export const moveNote: Actions["moveNote"] = action(async (note, column, order, timestamp) => {
    "use server";
    const accountId = await getAuthUser();
    const mutation = {
        id: String(note),
...
export const renameColumn: Actions["renameColumn"] = action(async (
    id: string,
    name: string,
) => {
    "use server";
...
export const createColumn: Actions["createColumn"] = action(async (
    id: string,
    board: string,
    name: string,
) => {
...
export const moveColumn: Actions["moveColumn"] = action(async (
    id,
    order,
    timestamp,
) => {
...
export const deleteColumn: Actions["deleteColumn"] = action(async (
    id,
    timestamp,
) => {
    "use server";
